...
const agContract = new cav.klay.Contract(DEPLOYED_ABI, DEPLOYED_ADDRESS); //bapp내에서 쓸 수 있는 전역 상수(abi ,address) 설정은 webpack에서 해줌
...
const App = {
    ...
    handleImport: async function () {
        ...
        fileReader.onload = event => {
            try {
                ...
                // 검증을 통과를 하게되면 keystore 파일의 내용을 전역변수에 저장
                $('#message').text('유효하지 않은 keysotre 파일 입니다.');
                return;
            }
        };
    ...
    handlePassword: async function () {
        // 비밀번호 체크 후 전역변수 auth.password 에 저장
        this.auth.password = event.target.value;
    },

    handleLogin: async function () {
        // keystore과 비밀번호를 통해서 얻을 수 있는 것 : private key
        if (this.auth.accessType === 'keystore') {
            //나중에 프라이빗 키를 넣어야 할 때는 변경하여 사용
            try {
    ...
    deposit: async function () {
        ...
        if (walletInstance) {
            ...
            //if ((await this.callOwner()).toLowerCase() === walletInstance.address)
                ...
                return; // amount가 없다면 함수 종료 하는 return 문
            }
        }
    },
    ...
    integrateWallet: function (privateKey) {
        //인자로 받은 프라이빗 키로 월렛 인스턴스를 가져옴 / 계정인증완료
    ...
    changeUI: async function (walletInstance) {
        ...
        console.log((await this.callOwner()).toLowerCase());
